...
uses
  System.Classes, System.SysUtils, System.Generics.Collections, FMX.Types, FMX.WebBrowser;
...
type
  ...
  IScriptGate = interface
    ...
    procedure CallScript(const iScript: String; const iResultProc: TScriptGateResultProc);
    procedure Eval(const iScript: String; const iResultProc: TScriptGateResultProc);
  end;
  ...
  public
    function CreateScriptGate(const IScriptGate: TScriptGate): IScriptGate; virtual; abstract;
  end;

  TScriptGateBase = class(TInterfacedObject, IScriptGate)
  public
    procedure CallScript(const iScript: String; const iResultProc: TScriptGateResultProc); virtual; abstract;
    procedure Eval(const iScript: String; const iResultProc: TScriptGateResultProc); virtual;
  end;
  ...
  public type
    ...
    TCheckUrlEvent = procedure(Sender: TScriptGate; const iUrl: String; var ioDone: Boolean) of object;
  ...
  public
    ...
    property OnBeforeCalled: TNotifyEvent read FOnBeforeCalled write FOnBeforeCalled;
    property OnAfterCalled: TNotifyEvent read FOnAfterCalled write FOnAfterCalled;
    property OnCallError: TCallErrorEvent read FOnCallError write FOnCallError;
  end;

implementation

uses
  System.Rtti, FMX.Platform, IdURI, SG.ScriptGateLog
{$IFDEF MSWINDOWS}
    , SG.ScriptGate.Win
{$ENDIF}
{$IFDEF OSX}
      ...
      begin
        ...
        begin
          ...
          case (RttiParams[i].ParamType.TypeKind) of
            TTypeKind.tkInteger, TTypeKind.tkInt64, TTypeKind.tkClass, TTypeKind.tkClassRef, TTypeKind.tkInterface,
              TTypeKind.tkPointer:
              try
...
procedure TScriptGate.CallScript(const iScript: String; const iResultProc: TScriptGateResultProc);
begin
  if (FGate = nil) then
  begin
...
constructor TScriptGate.Create(const iReceiver: TObject; const iWebBrowser: TWebBrowser; const iScheme: String);
var
...
begin
  ...
  if (TPlatformServices.Current.SupportsPlatformService(IScriptGateFactory, IInterface(GateFactory))) then
  begin
...
procedure TScriptGate.Eval(const iScript: String; const iResultProc: TScriptGateResultProc);
begin
...
procedure TScriptGateBase.Eval(const iScript: String; const iResultProc: TScriptGateResultProc);
var
  InSQuote: Boolean;
  SB: TStringBuilder;
  C: Char;
